...
public interface AppPunishmentRepository extends JpaRepository<AppPunishment, Long> {
    ...
    @Query(value = "SELECT * FROM app_punishment t where t.block_id = ?1 and t.punish_method = ?2 order by seal_up_time_begin desc", nativeQuery = true)
    List<AppPunishment> findSealUpByCondition(Long blockId, String punishMehtod);

    @Query(value = "SELECT * FROM app_punishment t where t.block_id = ?1 and t.punish_method in ?2 order by execute_time desc", nativeQuery = true)
    List<AppPunishment> findStopDataByCondition(Long blockId, List<String> methods);
    ...
    @Query(value = "SELECT * FROM app_punishment t where t.block_id = ?1 and t.punish_method = ?2 order by execute_time desc", nativeQuery = true)
    List<AppPunishment> findByBlockIdAndPunishMethod(Long bloclId, String punishMehtod);
